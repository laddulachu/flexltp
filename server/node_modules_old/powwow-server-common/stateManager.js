        ...
        if (arrInjectFiles.length > 0) {
            ...
            page.evaluate(function(nextEntry) {
                try {
                    ...
                    if (nextEntry.type && window.powwow && window.powwow[nextEntry.type]) {
                        if(nextEntry.type == "descriptors") {
                            // Also save an array of descriptors in the order loaded.  We'll
                            // use this later to make the state descriptors once they are all
                            // loaded.
                            exports.id = nextEntry.id;
                            window.powwow._listDescriptors.push(exports);
                        }
                        console.log("Setting", nextEntry.type, "with id", nextEntry.id);
            ...
            page.evaluate(function() {
                if(window.powwow) {
                    ...
                    // 2) Resolve any references to other descriptors (since we are traversing them anyway)
                    for(i=0; i < window.powwow._listDescriptors.length; i++) {
                        if(pageModelHasIdentifiersToRun(window.powwow._listDescriptors[i])) {
                            var pageIdentifier = {};
                            copyOverPageModelIdentifiersAndResolveSubDescriptors(window.powwow._listDescriptors[i], pageIdentifier, null, window.powwow.descriptors, window.powwow._listDescriptors[i]);
                            window.powwow.stateDescriptors.push(pageIdentifier);
                        }
                    }
                    delete window.powwow._listDescriptors;

                    function copyOverPageModelIdentifiersAndResolveSubDescriptors(node, nodetoCopyTo, id, mapPageModels, nodeParent) {
                        if(node.type == "object") {
                            ...
                            for(var prop in node.properties) {
                                copyOverPageModelIdentifiersAndResolveSubDescriptors(node.properties[prop], nodetoCopyTo.properties, prop, mapPageModels, node.properties);
                            }
                            ...
                            // Recurse over the properties to look for identifiers.
                            copyOverPageModelIdentifiersAndResolveSubDescriptors(node.items, nodetoCopyTo[id].items, prop, mapPageModels, node.items);
                            ...
                            nodetoCopyTo[id] = {};
                            copyOverPageModelIdentifiersAndResolveSubDescriptors(node, nodetoCopyTo[id], id, mapPageModels, node);
                        }
                    }
    ...
    page.evaluate(function() {
        ...
        if(needsInjection) {
            ...
            page.evaluate(function(env) {
                ...
                window.powwow._listDescriptors = [];
                ...
                // State descriptors have been removed, replaced with identifiers in main descriptor.
