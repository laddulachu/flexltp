    ...
    phantom.stdout.once('data', function (data) {
      ...
      // - this is only necessary when using cluster, but it's here regardless
		...
		function getWebServerPort() {
			logger.info("Calling", phantom_pid_command);
			exec(phantom_pid_command, function (err, stdout /*, stderr*/) {
			  if (err !== null) {
				if(ws_port_retry_count < ws_port_max_retries) {
					ws_port_retry_count++;
					logger.error('Error executing command to extract phantom ports: ', err);
			  ...
			  if (!port) {
				...
				return;
				  ...
				  logger.info("Using phantom port", port);
			  }
			...
			if (phantom.killed) {
			  ...
			  return;
				...
				if(num_retries < max_request_retries) {
					logger.warn('RETRYING - Request() error evaluating', method, '() call:', err);
					num_retries++;
