...
// Max interval without requests from master process - set to 30 minutes.
...
function watchdog_clear() {
  ...
  watchdog_timer_id = setTimeout(function () {
    system.stderr.writeLine("Watchdog timer expired");
    phantom.exit(0);
  }, WATCHDOG_TIMEOUT);
}
...
function setup_callbacks (id, page) {
  callbacks.forEach(function (cb) {
    page[cb] = function (parm) {
      ...
      if (cb === 'onClosing') { args = [{url: parm.url}]; }
      callback_stack.push({ 'page_id': id, 'callback': cb, 'args': args });
    };
  });
  // Special case this
  page.onPageCreated = function (page) {
    // Workaround for https://github.com/ariya/phantomjs/issues/12547
    page.customHeaders = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:46.0) Gecko/20100101 Firefox/46.0' };
