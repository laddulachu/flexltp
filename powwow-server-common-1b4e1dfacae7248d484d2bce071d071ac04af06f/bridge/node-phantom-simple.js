...
var POLL_INTERVAL   = process.env.POLL_INTERVAL || 150;
...
exports.create = function (options, callback) {
  ...
  function spawnPhantom (callback) {
    ...
    if(options.env) {
      ...
      for(var envVar in options.env) {
        env[envVar] = options.env[envVar];
      }
      spawnOptions = { env: env };
    }

    var phantom = spawn(options.path, args, spawnOptions);
    ...
    phantom.stdout.once('data', function (data) {
      ...
      // Now need to figure out what port it's listening on.  This requires checking with the OS
      // due to: https://github.com/ariya/phantomjs/issues/13947
      // We need to use lsof on Mac, netstat on Windows and ss/netstat on Linux.
      ...
      switch (platform) {
        ...
        case 'win32':
	        cmd = 'netstat -ano | findstr /R /C:"TCP.*LISTENING[ ]*%d$"';
          break;
      ...
      // - this is only necessary when using cluster, but it's here regardless
		...
		var ws_port_max_retries = 20;
		var ws_port_retry_timeout_ms = 5000;
		var ws_port_retry_count = 0;

		function getWebServerPort() {
			console.log("Calling", phantom_pid_command);
			exec(phantom_pid_command, function (err, stdout /*, stderr*/) {
			  if (err !== null) {
				if(ws_port_retry_count < ws_port_max_retries) {
					ws_port_retry_count++;
					console.log('Error executing command to extract phantom ports: ', err);
					console.log('Trying again...');
					setTimeout(getWebServerPort, ws_port_retry_timeout_ms);
				} else {
					phantom.kill();
			  ...
			  if (!port) {
				...
				return;
				  ...
				  console.log("Using phantom port", port);
			  }
		  ...
		  req.on('error', function (err) {
			...
			if (phantom.killed) {
			  ...
			  return;
				...
				// Retry the request.
				if(num_retries < max_request_retries) {
					console.log('RETRYING - Request() error evaluating', method, '() call:', err);
					num_retries++;
					setTimeout(makeRequest, 500);
				} else {
					console.log('Request() error evaluating', method, '() call:', err);
...
function setup_long_poll (phantom, port, pages, setup_new_page) {
  ...
  var poll_func = function (cb) {
    ...
    var req = http.get(http_opts, function(res) {
      ...
      res.on('end', function () {
        ...
        if (dead) {
          cb(new HeadlessError('Phantom Process died'));
          process.stdout.write("Phantom Process died\n");
		  process.exit();
          return;
        }
    ...
    req.on('error', function (err) {
      if (dead || phantom.killed) {
          process.stdout.write(JSON.stringify(err, 0, 2));
          process.stdout.write("Phantom was killed 1\n");
          process.exit();
          return;
      }
      ...
      if (err.code === 'ECONNRESET' || err.code === 'ECONNREFUSED') {
        ...
        cb(new HeadlessError('Phantom Process died'));
        process.stdout.write(JSON.stringify(err, 0, 2));
        process.stdout.write("Phantom was killed 2\n");
        process.exit()
        return;
      }
  ...
  var repeater = function () {
    ...
    if (dead || phantom.killed) {
      process.stdout.write("Phantom Exiting\n");
      process.exit();
      return;
    }
